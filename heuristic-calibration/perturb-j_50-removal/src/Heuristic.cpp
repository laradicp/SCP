...
{
    int t = 0, sSize = s.size();
    int n = sSize/2 < data.getDimension()/50 ? sSize/2 : data.getDimension()/50;
    ...
    else // swap
    {
        ...
        {
            int p1 = rand()%sSize;
            while(infeasibleSwapPos[p1] == '1') // it will find a '0' because n = min(sSize/2, data.getDimension()/50)
            {
                if(++p1 == sSize)
                {
                    p1 = 0;
