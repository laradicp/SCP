...
class Data
{
    private:
        ...
        int maxCadence;
        std::vector<std::pair<std::vector<char>, std::vector<int>>> families;
        int cadencesSize;
        int familiesSize;
        std::vector<int> familySize;
        std::vector<int> lb;

        static bool compareCadencePairs(std::pair<std::pair<int, int>, char> p1, std::pair<std::pair<int, int>, char> p2);
